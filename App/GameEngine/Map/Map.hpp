...
namespace engine {
    ...
    class Map {
        public:
            Map() : distance(0), bossTrigger(5000), deadBackLine(-1000), deadFrontLine(2920), spawnLine(2220), lifeZone(engine::Square(::game::Vector2(-500, -200), 2920, 1480)) {
        ...
        private:
            ...
            void fill(std::vector<SpawnElement> &spawns, size_t length) {
                std::vector<unsigned int> enemy {14, 21, 22, 23, 24};
                for (size_t i = 0; i < length; ++i) {
                    while(rand() % 100 < 55) {
                        if (rand() % 100 < 30) {
                            SpawnElement element;
                            element.distance = i * 100;
                            element.pos = ::game::Vector2(this->spawnLine, /*random*/ 40 + rand() % 900);
                            element.delay = 3.f;
                            element.repeat = 1;
                            // element.repeatType = 1;
                            int rnd = rand() % enemy.size();
                            element.entityType = enemy[rnd];
