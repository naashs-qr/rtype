...
class GameManager {
    private:
        GameManager() : _push(false), _switch(false), _quit(false), _playerNumber(4), _lvl(0), _delayTransition(-1) {
    ...
    private:
        ...
        static void initWinLoose();

        static void loopMenu(::engine::PollEvent &event);
        static void loopGame(::engine::PollEvent &event);
        static void loopWinLoose(::engine::PollEvent &event);

        static void manageLevel(engine::Boss *boss);


        void initWindowStack() {
            this->_windows.push(WindowState::WINLOOSE);
        ...
        void initWindowSwitcher() {
            ...
            this->_switchToWindow.insert(std::make_pair(WindowState::WINLOOSE, std::make_pair(
                &GameManager::initWinLoose,
                &GameManager::loopWinLoose)));
        }
